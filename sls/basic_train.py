...
def loss_batch(model:nn.Module, xb:Tensor, yb:Tensor, loss_func:OptLossFunc=None, opt:OptOptimizer=None,
    ...
    loss = loss_func(model(*xb),*yb) #call one more time for updating metrics from SLS 
...
@dataclass
class Learner():
    "DefaultTrainer for `model` using `data` to minimize `loss_func` with optimizer `opt_func`."
    data:DataBunch
    model:nn.Module
